...
app = FastAPI(title="CAIO Backend", version="0.3.0")
init_db()
...
if extra: ALLOWED_ORIGINS += [o.strip() for o in extra.split(",") if o.strip()]
app.add_middleware(CORSMiddleware, allow_origins=ALLOWED_ORIGINS, allow_credentials=True, allow_methods=["*"], allow_headers=["*"], expose_headers=["*"], max_age=86400)

@app.options("/{path:path}")
def cors_preflight(path: str): return JSONResponse({"ok": True})
...
@app.get("/api/ready")
def ready(): return {"ready":True,"time":datetime.utcnow().isoformat()+"Z"}
...
def _login_core(email: str, password: str, db: Session):
    ...
    if user:
        if not hasattr(user, "hashed_password"): raise RuntimeError("User model missing 'hashed_password'")
        if not verify_password(password, user.hashed_password): raise HTTPException(status_code=400, detail="Incorrect email or password")
    else:
        hpw = get_password_hash(password)
        user = User(email=email, hashed_password=hpw if hasattr(User,"hashed_password") else None,
    ...
    return {"access_token":token,"token_type":"bearer","email":user.email,"is_admin":bool(getattr(user,"is_admin",False)),"is_paid":bool(getattr(user,"is_paid",False))}

@app.post("/api/login")
def login(form: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    try: return _login_core((form.username or "").strip().lower(), form.password or "", db)
    except HTTPException: raise
    except Exception as e:
        logger.error("Login failed: %s\n%s", e, traceback.format_exc())
        if DEBUG: return JSONResponse({"detail": f"login-500: {e}"}, status_code=500)
        raise HTTPException(status_code=500, detail="Login failed")

@app.get("/api/profile")
def profile(current_user: User = Depends(get_current_user)):
    email = current_user.email.lower()
    tier = "admin" if email in ADMIN_EMAILS else ("premium" if email in PREMIUM_EMAILS else ("pro" if getattr(current_user,"is_paid",False) else "demo"))
    return {"email": current_user.email, "is_admin": email in ADMIN_EMAILS, "is_paid": bool(getattr(current_user,"is_paid",False)), "created_at": getattr(current_user,"created_at",None), "tier": tier}
...
def _check_and_increment_usage(db: Session, user: User, endpoint: str = "analyze"):
    ...
    if limit is None:
        db.add(UsageLog(user_id=getattr(user,"id",0) or 0, timestamp=datetime.utcnow(), endpoint=endpoint, status="ok")); db.commit()
        return True, 0, None, end, tier
    used = (db.query(UsageLog).filter(UsageLog.user_id==getattr(user,"id",0)).filter(UsageLog.endpoint==endpoint)
            .filter(UsageLog.timestamp>=start).filter(UsageLog.timestamp<=end).count())
    if used >= limit: return False, used, limit, end, tier
    db.add(UsageLog(user_id=getattr(user,"id",0) or 0, timestamp=datetime.utcnow(), endpoint=endpoint, status="ok")); db.commit()
...
def _limit_response(used: int, limit: int, reset_at: datetime, tier: str):
    ...
    return JSONResponse(status_code=429, content={
        "status":"error","title":"Daily limit reached",
        "message":f"You've used {used}/{limit} {plan} requests today. Resets at {reset_at.strftime('%H:%M')} UTC.",
        "plan":tier,"used":used,"limit":limit,"remaining":max(0,limit-used),"reset_at":reset_at.isoformat()+"Z",
    })
...
async def analyze(request: Request, file: Optional[UploadFile]=File(None), text: Optional[str]=Form(None), brains: Optional[str]=Form(None),
                  current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    allowed, used, limit, reset_at, tier = _check_and_increment_usage(db, current_user, endpoint="analyze")
    if not allowed: return _limit_response(used, int(limit or 0), reset_at, tier)
    if tier == "demo":
        chosen = _choose_brains(brains, is_paid=False)
        return JSONResponse({"status":"demo","title":f"Demo Mode · {', '.join(chosen)}",
                             "summary":"This is a demo preview. Upgrade to Pro to run all brains with real analysis.\n"
                                      f"Brains used: {', '.join(chosen)}","tip":"Upload a business document or provide a brief to see the flow."})
    try: extracted = await _extract_text(file)
    except Exception as e: logger.error("Extraction failed: %s", e); extracted = ""
    brief = (text or "").strip()
    chosen = _choose_brains(brains, is_paid=True)
    if not extracted and not brief: raise HTTPException(status_code=400, detail="Please upload a file or provide text")
    ...
    if provider_chain:
        for p in provider_chain:
            try:
                if p=="openai": summaries = {b:_call_openai_prompt(pr) for b,pr in prompts.items()}
                elif p=="openrouter": summaries = {b:_call_openrouter_prompt(pr) for b,pr in prompts.items()}
                used_provider = p; break
            except Exception as e: logger.error("Provider %s failed: %s", p, e); errs.append(f"{p}:{e.__class__.__name__}"); summaries={}
    if not summaries: summaries = {b:f"[Stub after error] {b} analysis. Errors: {', '.join(errs) if errs else 'no-provider'}." for b in prompts.keys()}
...
def _llm_chat(messages: List[dict]) -> str:
    ...
    if "openai" in providers:
        ...
        r = requests.post("https://api.openai.com/v1/chat/completions", headers=headers, data=json.dumps(data), timeout=60)
        if r.status_code<400: return r.json()["choices"][0]["message"]["content"]
...
def _history(db: Session, session_id: int, user_id: int, limit: int = 20) -> List[ChatMessage]:
    q = (db.query(ChatMessage).join(ChatSession, ChatMessage.session_id==ChatSession.id)
...
async def chat_send(
    ...
    db.add(UsageLog(user_id=getattr(current_user,"id",0) or 0, timestamp=datetime.utcnow(), endpoint="chat", status="ok")); db.commit()
    ...
    context = ""
    try: context = (await _extract_text(file)).strip() if file else ""
    except Exception as e: logger.warning("File read failed: %s", e); context=""
...
def chat_history(session_id: int = Query(...), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if not _is_premium_or_admin(current_user):
        raise HTTPException(status_code=403, detail="Chat is a Premium feature.")
    sess = db.query(ChatSession).filter(ChatSession.id==session_id, ChatSession.user_id==getattr(current_user,"id",0)).first()
    if not sess: raise HTTPException(status_code=404, detail="Session not found")
    items = [{"id":m.id,"role":m.role,"content":m.content,"created_at":m.created_at.isoformat()+"Z"} for m in _history(db, session_id, getattr(current_user,"id",0), limit=100)]
...
def chat_sessions(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if not _is_premium_or_admin(current_user):
        raise HTTPException(status_code=403, detail="Chat is a Premium feature.")
    rows = (db.query(ChatSession).filter(ChatSession.user_id==getattr(current_user,"id",0)).order_by(ChatSession.id.desc()).limit(50).all())
...
def ping_db(db: Session = Depends(get_db)):
    try:
        ...
        if DEBUG: return JSONResponse({"ok": False, "detail": str(e)}, status_code=500)
